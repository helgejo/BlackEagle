...
% Bruker input
plotFrek=input('Tast inn et tall fra 0 til 100000. Høyere tall gir bedre styring via joystick, mens lavere gir oftere oppdatering av figurere og grafer: ');
...
figure(1); %Main figur, Totalt avvik, Rettning på nåværende avvik, Verdi, delta T
figure(2); %Motor figur, pådrag høyre motor, pådrag venstre motor
...
while run
    ...
    joyFB(end+1) = -joystick.axes(2)/327.68; % henter joystick posisjon på "y"-aksen
    joyS(end+1) = joystick.axes(1)/327.68; % henter joystick posissjon på "x"-aksen
    
    %Filtrere joystick signalet
    joyFB(end) = filtJoy([joyFB(end-1),joyFB(end)]);
    joyS(end) = filtJoy([joyS(end-1),joyS(end)]);
    
    %beregner motor pådrag
